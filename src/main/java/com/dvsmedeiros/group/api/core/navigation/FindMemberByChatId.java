...
@Component
public class FindMemberByChatId implements IStrategy<Filter<Member>>{
	...
	public void process(Filter<Member> aEntity, INavigationCase<Filter<Member>> aCase) {
		...
		List<Member> memberList = memberRepository.findByChatList(aEntity.getEntity().getChatList());
